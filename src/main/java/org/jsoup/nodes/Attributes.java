...
/**
 * 用来表示元素的属性值，属性值的结构类似于Map，一个属性名字只能对应一个属性值。（这二货作者自己实现了Map的逻辑，但是却是数组的效率）
 * 通常情况下属性名是大小写敏感的，但是这里在解析的时候把属性名字都处理为小写的了，所以可以使用小写的属性名字。
...
public class Attributes implements Iterable<Attribute>, Cloneable {
	...
	// 用来存储数据的属性名前缀，可以使用data-后面跟着的名字来获取其值，是h5的一个规范
	// 在jsoup中可以使用element.data('attr-name')来获取
	protected static final String dataPrefix = "data-";
	private static final int InitialCapacity = 4; // todo - analyze Alexa 1MM sites, determine best setting

	// manages the key/val arrays 增长因子，每次扩充倍数
	private static final int GrowthFactor = 2;
	private static final String[] Empty = {};
	static final int NotFound = -1;
	private static final String EmptyString = "";

	// 如果是从前往后挨个存的话，那么需要有一个变量来标记下一次放在哪个位置
	...
	private static String[] copyOf(String[] orig, int size) {
		...
		// 如果不需要收缩的话没有必要找较小的一个 Math.min(orig.length, size);
...
//    }
	...
	int indexOfKey(String key) {
		return indexOfKeyByEqualsPredicate(key, StringUtil::equals);
	}

	private int indexOfKeyIgnoreCase(String key) {
		return indexOfKeyByEqualsPredicate(key, StringUtil::equalsIgnoreCase);
	}

	private int indexOfKeyByEqualsPredicate(String key, BiPredicate<String, String> predicate) {
		Validate.notNull(key);
		for (int i = 0; i < size; i++) {
			if (predicate.test(key, keys[i]))
				return i;
		}
		return NotFound;
	}
	 ...
	 * @return these attributes, for chaining
	 */
	Attributes truePutFalseRemove(String key, boolean value) {
		if (value)
	...
	public boolean equals(Object o) {
		...
		return size == that.size && Arrays.equals(keys, that.keys) && Arrays.equals(vals, that.vals);
	}
