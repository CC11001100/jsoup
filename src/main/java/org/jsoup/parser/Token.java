...
abstract class Token {
	...
	static abstract class Tag extends Token {
		protected String tagName; // 原始的标签名，不一定是小写
		protected String normalName; // lowercase version of tag name, for case insensitive tree build
			...
			if (pendingAttributeName != null) {
				...
				if (pendingAttributeName.length() > 0) {
					...
					if (hasPendingAttributeValue)
						value = pendingAttributeValue.length() > 0 ?
		...
		@SuppressWarnings({ "TypeMayBeWeakened" }) final Attributes getAttributes() {
			return attributes;
		}
